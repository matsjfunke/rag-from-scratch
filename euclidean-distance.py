...
def get_similar_chunks(embeddings, prompt_embedding):
    ...
    for index, embedding_data in enumerate(embeddings):
        ...
        for dimension in range(len(embedding_vector)):
            ...
            before_square_root += (embedding_vector[dimension] - prompt_embedding["embedding"][dimension])**2

            elucidean_distance = np.sqrt(before_square_root)

        similarity_list.append((elucidean_distance, index))
...
def main():
    ...
    prompt = "Where does the book take place?"  # input("enter a question: ")
    ...
    for chunk in similar_chunks:
        print(f"This is the Elucidean Distance:{chunk[0]} of chunk: {chunks[chunk[1]]}\n")
